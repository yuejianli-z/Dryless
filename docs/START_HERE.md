...
## Windows 已实施更新（rc.5）
顶部摄像头/声音统一为“名称：开/关”，同尺寸同样式，删除它们与Tips的说明悬停框。
新增现有原眼睛的匹配闭眼资源：运行睁眼，未运行闭眼，页面/侧栏/任务栏/托盘同步；暂停不等于关闭。
Tips独立卡片沿用整体底色与文字色，无另设绿色描边，远眺为连续地平线图标。仍为三条短文案、20秒自动轮播。
见 [release/windows-rc5.md](release/windows-rc5.md)。保留检测、统计、声音、20分钟微休息与三页结构；F2缺口未因此完成。
...
版本：version.py；候选：0.2.0-rc.5；正式目标：0.2.0。
