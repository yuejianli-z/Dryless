...
## Windows 已实施更新（rc.4）
在rc.3摄像头/托盘功能基础上，摄像头与声音开关统一置于所有页面顶部、等尺寸。
监测页右侧上方是状态与指标；下方是独立浅绿Tips卡片，三条图标/标题/短句每20秒轮播，悬停/焦点/提醒时暂停，无额外按钮。
暂停提醒在状态旁，删除重复设置入口。统计气泡图、原logo、已选声音与20分钟微休息保留。
本轮范围与验证见 [release/windows-rc4.md](release/windows-rc4.md)。Mac保持原生UI，按共享功能合同对齐；F2仍有未完成项。
...
版本：version.py；候选：0.2.0-rc.4；正式目标：0.2.0。
