...
## Windows 已实施更新（rc.8）
清除自动悬停窗并加入全局拦截；气泡明细固定显示在图内。
顶部摄像头/声音为图标按钮，关闭时加斜线，与Language等高；中文标语为“多眨眼，少干涩”。
频率按同期有效识别秒数折算：30秒10次=20次/分钟。检测算法保持原样。
原版音色显示名改为Ding；点音色试听首档一次，保留“试听三档”。
保留rc.7左侧固定提醒卡与rc.6实时指标。见 [release/windows-rc8.md](release/windows-rc8.md)。
F2其余缺口不因此标为完成。
...
版本：version.py；候选：0.2.0-rc.8；正式目标：0.2.0。
