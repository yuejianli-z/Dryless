...
## Windows 已实施更新（rc.7）
提醒已从挤压主界面的底部横条移到左侧固定卡片区域；三页、双语、三档提醒及收起/暂停/微休息均不改变页面尺寸。
保留rc.6的60秒滑动频率、未眨眼进度、连续在场微休息进度与柔和统一配色。
见 [release/windows-rc7.md](release/windows-rc7.md) 与 [release/windows-rc6.md](release/windows-rc6.md)。F2其余缺口不因此标为完成。
...
版本：version.py；候选：0.2.0-rc.7；正式目标：0.2.0。
