...
## Windows 已实施更新（rc.6）
首页增加最近60秒滑动频率圆环、距上次眨眼进度与真实连续在场的微休息进度；替换原大号累计指标。
主频率与下方完整分钟采用相同有效秒数口径，三处图表共用柔和红黄绿；缺测留空、真实零保留、不截断到30。
节律色块及颜色图例不弹悬停窗，精确分钟值可在图内查看。其余沿用rc.5的顶部控件、眼睛状态和Tips。
见 [release/windows-rc6.md](release/windows-rc6.md)；新增实时指标合同见FEATURE_PARITY.md第3节。F2其余缺口不因此标为完成。
...
版本：version.py；候选：0.2.0-rc.6；正式目标：0.2.0。
