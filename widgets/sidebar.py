...
from i18n import t
from project_links import open_project
...
class Sidebar(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._status_tile = QFrame()
        self._status_tile.setObjectName("SidebarStatusTile")
        # This is a read-only signal, not a second action. Keep the live state
        # visually centred while reserving the filled treatment for GitHub.
        self._status_tile.setFixedHeight(28)
        self._status_tile.setStyleSheet(
            "QFrame#SidebarStatusTile{background:transparent;border:none;}")
        face_row = QHBoxLayout(self._status_tile)
        face_row.setContentsMargins(0, 0, 0, 0)
        face_row.setSpacing(6)
        face_row.addStretch(1)
        self._face_dot = StatusLight(size=16)
        face_row.addWidget(self._face_dot)
        self._face_label = QLabel(t("status_waiting"))
        self._face_label.setFont(T.ui_font(12))
        self._face_label.setFixedHeight(18)
        self._face_label.setSizePolicy(QSizePolicy.Policy.Preferred, QSizePolicy.Policy.Fixed)
        ...
        face_row.addStretch(1)
        bot_lay.addWidget(self._status_tile)

        self._project_button = QPushButton("Star on GitHub")
        self._project_button.setFont(T.ui_font(13))
        self._project_button.setFixedHeight(36)
        self._project_button.setIcon(icon("github", 18))
        self._project_button.setIconSize(QSize(18, 18))
        self._project_button.setCursor(Qt.CursorShape.PointingHandCursor)
        self._project_button.setAccessibleName("Star on GitHub")
        self._project_button.setAccessibleDescription("Open the Dryless project in your browser")
        self._project_button.setStyleSheet(
            f"QPushButton{{color:{T.S_TEXT};background:{T.C_CARD};border:none;outline:none;"
            "border-radius:8px;padding:0 6px;text-align:center;}"
            f"QPushButton:hover{{background:{T.S_HOVER};border:none;}}"
            f"QPushButton:focus{{background:{T.BRAND_SOFT};border:none;outline:none;}}"
            f"QPushButton:pressed{{background:{T.S_ACTIVE};border:none;}}")
        self._project_button.clicked.connect(lambda: self._open_project())
        bot_lay.addWidget(self._project_button)
    ...
    def _open_project(self):
        if open_project():
            self._project_button.setText("Star on GitHub")
        else:
            from config import LANGUAGE
            self._project_button.setText("重试打开 GitHub" if LANGUAGE == "zh" else "Retry GitHub")
